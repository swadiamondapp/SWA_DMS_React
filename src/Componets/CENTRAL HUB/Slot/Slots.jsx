      ...
      <div className="Users_Table_List">
        <table style={{ width: "100%" }}>
          ...
          <Modal open={printSlotModalOpen} onClose={handlePrintSlotModalClose}>
            <Box sx={printSlotOpen} style={{ position: 'relative', maxHeight: '80vh', overflow: 'hidden' }}>
              <div className="headerModal" style={{ position: "sticky", top: 0, background: "#F2F2F2", padding: "5px 10px", zIndex: 1000 }}>
              ...
              <Typography style={{ overflowY: 'auto', maxHeight: 'calc(70vh - 50px)', padding: '10px 0',paddingBottom:"100px" }}>
                <div className="print_slot_modal_container">
                  <div className="Slote_Container">
                    <table className="custom-table single-border">
                      <thead>
                        <tr>
                          <th className="column-header">Product ID</th>
                          <th className="column-header">Created date</th>
                          <th className="column-header">Product Category</th>
                          <th className="column-header">Weight</th>
                        </tr>
                      </thead>
                      <tbody className="tbodyy" style={{paddingBottom:"10px"}}>
                        {slotView.map((item, index) =>
                          item.caddesigns.map((design, designIndex) => (
                            <tr key={`${index}-${designIndex}`}>
                              ...
                              <td className="table-cell">{design.product_category.join(", ")}</td>
                              <td className="table-cell">{design.approx_metal_weight} Gram</td>
                            </tr>
                          ))
                        )}
                      </tbody>
